...
void SnowHeight::Interface::Usart::Transmit(long number)
{
	...
	*/
	...
	else
	{
		...
		{
			negativeNumber = true;
			remainder *= -1;	// convert the negative number into a positive one for character conversion (while())
		}
		...
		if (negativeNumber)
		{
			...
			decimalPlace++; // set to the next field of array "valueAsASCII[20]
		}
	}
...
void SnowHeight::Interface::Usart::Transmit(int64_t number)
{
	long remainder = number;
	int8_t rest = 0;
	char valueAsASCII[20] = {0};	// value as ASCII-string representation with 20 digits incl. sign
	uint8_t decimalPlace = 0;
	bool negativeNumber = false;
